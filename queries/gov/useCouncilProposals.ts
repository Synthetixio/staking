...
import { COUNCIL_INDIVIDUAL_PROPOSAL, COUNCIL_PROPOSALS } from 'constants/snapshot';
...
const useCouncilProposals = (options?: QueryConfig<Proposal[]>) => {
	...
	return useQuery<Proposal[]>(
		QUERY_KEYS.Gov.CouncilProposals(walletAddress ?? '', network?.id!),
		async () => {
			let proposalsResponse = await axios.get(COUNCIL_PROPOSALS);
			...
			for (var key in data) {
				...
				const hash = key;
				let voterResponse = await axios.get(COUNCIL_INDIVIDUAL_PROPOSAL(hash));
...
export default useCouncilProposals;
